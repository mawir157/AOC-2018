...
func Cave(widthX int, widthY int, 
...
func minMap(set map[Pos]bool, dist map[Pos]int) (pMin Pos) {
	min := -1 //  needs to be bigger than infinity
...
func minAlt(set map[Pos]bool, flagged map[Pos]int) (pMin Pos) {
	min := -1 //  needs to be bigger than infinity
...
func Dijkstra(grid [][]int, source Pos, target Pos) (dist map[Pos]int, prev map[Pos]Pos) {
	Q := make(map[Pos]bool)
	...
	for x := 0; x < xLim; x++ {
		for y := 0; y < yLim; y++ {
			for g := 0; g < 3; g ++ {
				if (g != grid[x][y]) { // can never hit one third of the cave
	...
	for len(Q) > 0 {
		if len(marked) == 0 { // there are no more places to move to
			return
		}
		// u := minMap(Q, dist)
		u := minAlt(Q, marked)
...
func main() {
	...
	ds, _ := Dijkstra(grid, start, target)
